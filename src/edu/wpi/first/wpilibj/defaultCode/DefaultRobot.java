...
//        }
    ...
    public void teleopPeriodic() {
        ...
        double joyAx2 = m_joystick.getRawAxis(2);
        double joyAx3 = m_joystick.getRawAxis(3);
        double joyAx4 = m_joystick.getRawAxis(4);
        
        if (Math.abs(joyAx2) < 0.15)
            joyAx2 = 0;
        if (Math.abs(joyAx3) < 0.15)
            joyAx3 = 0;
        if (Math.abs(joyAx4) < 0.15)
            joyAx4 = 0;
        double halfArm = 1.0;
        ...
        if(!override){
            if(arcadeDrive){
                m_robotDrive.arcadeDrive((1-2*m_buttons[6])*joyAx2*halfArm, joyAx4/(3+(m_buttons[7]*3))*2*halfArm, false);
            }
            else{
                if(m_buttons[6]==1)
                    m_robotDrive.tankDrive(-joyAx3, -joyAx2);
                else
                    m_robotDrive.tankDrive(joyAx2, joyAx3);
            }
        }
